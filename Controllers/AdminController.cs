...
public class AdminController : Controller
{
    ...
    public AdminController(ApplicationDbContext context, UserManager<VisitorModel> userManager, SignInManager<VisitorModel> signInManager, RoleManager<IdentityRole> roleManager, IAccountService accountService, IUserRedirectionService redirectionService)
    {
    ...
    public async Task<IActionResult> DeleteItemConfirmed(int id)
    {
        ...
        return RedirectToAction("IndexItem", "Employee");
    }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (abonnement != null)
                {
                    var user = new VisitorModel
                    {
                        ...
                        Email = model.Email,
                        Name = model.Naam, // Added this line
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> CreateLocation(LocationViewModel locatieViewModel)
    {
        if (ModelState.IsValid)
        {
            var locatieModel = new LocationModel
            {
                LocationName = locatieViewModel.LocationName
            };
    ...
    public async Task<IActionResult> EditLocation(string beschrijving)
    {
        ...
        var locatieModel = await _context.Locaties.FirstOrDefaultAsync(l => l.LocationName == beschrijving);
        if (locatieModel == null)
        {
            return NotFound();
        }

        var locatieViewModel = new LocationViewModel
        {
            LocationName = locatieModel.LocationName
        };
    ...
    public async Task<IActionResult> EditLocation(LocationViewModel locationViewModel)
    {
        if (ModelState.IsValid)
        {
            try
            {
                var locationModel = await _context.Locaties.FirstOrDefaultAsync(l => l.LocationName == locationViewModel.LocationName);
                if (locationModel == null)
                {
                    return NotFound();
                }

                // Update other properties of locationModel as needed
    ...
    public async Task<IActionResult> DeleteLocation(int? id)
    {
        ...
        var locationModel = await _context.Locaties
    ...
    public async Task<IActionResult> DeleteLocationConfirmed(int id)
    {
        ...
        var locationModel = await _context.Locaties.FindAsync(id);
        if (locationModel == null)
        {
            return NotFound();
        }

        _context.Locaties.Remove(locationModel);
        await _context.SaveChangesAsync();
        return RedirectToAction(nameof(IndexLocation));
    }
